...
df = pd.read_pickle('../../data/interim/data_resampled_outliers_removed_chauvenet.pkl')
...
subset = df_lowpass[df_lowpass['set'] == 90]
print(subset['label'][0])

fig, ax = plt.subplots(nrows=2, sharex=True, figsize=(20, 10))
ax[0].plot(subset['acc_y'].reset_index(drop=True), label='raw data')
ax[1].plot(subset['acc_y_lowpass'].reset_index(drop=True), label='lowpass filtered')
...
plt.plot(range(1, len(predictor_columns) + 1), pc_values, marker='o')
...
subset = df_squared[df_squared['set'] == 75]
...
for s in df_temporal['set'].unique():
    ...
    subset = num_abs.abstract_numerical(subset, predictor_columns, ws, 'mean')
    subset = num_abs.abstract_numerical(subset, predictor_columns, ws, 'std')
